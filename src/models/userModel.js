...
var userJSON = {
  ...
  firstName: { type: String, trim: true, required: true, minlength: 2, maxlength: 16 },
  lastName: { type: String, trim: true, required: true, minlength: 2, maxlength: 16 },
  // TODO: Add email format validation regex. Note: API is already protected
  emailId: { type: String, trim: true, required: true, unique: true, minlength: 4, maxlength: 64 },
  ...
  profile: {
    ...
    gender: { type: String, trim: true, uppercase: true, enum: ['MALE', 'FEMALE', 'OTHER'] },
    programsData: [{
      programId: { type: Number, required: true },

      status: { type: String, enum: ['COMPLETE', 'ACTIVE', 'UNLOCKED', 'LOCKED'], default: defaultsConfig.programsStatus().toUpperCase() },
      ...
      modules: [{
        ...
        status: { type: String, enum: ['COMPLETE', 'ACTIVE', 'UNLOCKED', 'LOCKED'], default: defaultsConfig.modulesStatus().toUpperCase() },
        read: { type: Boolean, default: false }, // setting this true or false shouldn't affect the selective forest walk
        ...
        activities: [{
          ...
          status: { type: String, enum: ['COMPLETE', 'ACTIVE', 'UNLOCKED', 'LOCKED'], default: defaultsConfig.activitiesStatus().toUpperCase() },
          updatedAt: { type: Date },
        }],
        tasks: [{
          taskId: { type: Number, required: true },
          status: { type: String, enum: ['COMPLETE', 'ACTIVE', 'UNLOCKED', 'LOCKED'], default: defaultsConfig.tasksStatus().toUpperCase() },
          updatedAt: { type: Date },
          submissions: [{
            createdAt: { type: Date, default: Date.now },
            data: [{
 ...
 * */
if ("test" === process.env.NODE_ENV) userJSON._id = { type: Schema.ObjectId, default: function () { return new mongoose.mongo.ObjectId() } }
